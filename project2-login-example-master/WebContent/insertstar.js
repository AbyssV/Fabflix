...
function handleAddResultt(resultDataJson) {
	console.log("in handle     "+resultDataJson);
	console.log("in this function");
	//resultDataJson = JSON.parse(resultData);
...
function submitLoginFormm(formSubmitEvent) {
	...
	jQuery.ajax({	  
		  ...
		  success: (resultData) => handleAddResultt(resultData),
		  error: function(xhr, status, error) {
			  var err = eval("(" + xhr.responseText + ")");
			  alert(err.Message);
}
...
// bind the submit action of the form to a handler function
jQuery("#input_form_movie").submit((event) => submitLoginFormm(event));
